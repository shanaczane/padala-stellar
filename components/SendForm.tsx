...
export function SendForm({ address, balance, networkPassphrase, onSuccess }: SendFormProps) {
  ...
  return (
    <div className="w-full rounded-xl border border-slate-800 bg-slate-900 p-6 shadow-lg">
      <h2 className="mb-4 text-sm font-medium uppercase tracking-wide text-slate-400">Send XLM</h2>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        <label className="flex flex-col gap-1 text-sm text-slate-400">
          ...
          <input
            ...
            placeholder="G..."
            className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 font-mono text-sm text-slate-100 disabled:opacity-50"
          />
        </label>
        ...
        <p className="text-xs text-slate-500">Spendable: {spendable.toFixed(2)} XLM</p>
